...
metadata:
  nanobot:
    ...
    tools:
      ...
      - thread_inspect        # Inspect thread history and sent items
      - memory                # Read and write long-term user preferences
capabilities:
  email_accounts:
...
| `bodyMaxChars` | 1500 | 首轮分诊正文截断字符数（账号级上限 12000） |
...
7. **时间边界约束（强制）**
- 先从首个成功的 `email_fetch` 输出中读取顶层 `anchor_date` 字段作为唯一锚点日（格式 `YYYY-MM-DD`）；该字段由脚本在首封有效邮件的 `date` 中截取日期部分写入；本轮所有后续计算都复用该锚点，禁止中途重新取"今天"。
